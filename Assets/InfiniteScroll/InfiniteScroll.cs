...
public class InfiniteScroll : UIBehaviour
{
	...
	protected override void Start ()
	{
		...
		for (int i=0; i<m_instantateItemCount; i++) {
			...
			item.anchoredPosition = 
					...
					new Vector2 (0, -ItemScale * DistanceOffset * (i)) : 
					new Vector2 (ItemScale * DistanceOffset *  (i), 0) ;
			m_itemList.Add (item);
	...
	void Update ()
	{
		...
		while (AnchoredPosition - m_diffPreFramePosition  < -ItemScale * 2) {
			...
			item.anchoredPosition = (direction == Direction.Vertical ) ? new Vector2 (0, -pos * DistanceOffset) : new Vector2 (pos * DistanceOffset, 0);
